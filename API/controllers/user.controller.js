...
async function createUser(req, res) {
    ...
    try {
        ...
        if (phone.length !== 9 || isNaN(phone)) {
            return res.status(422).json({ message: "Phone number must be 9 digits long" });
        }

        // Verifica se o email e o telefone são únicos
        const existingUser = await models.user.findOne({ where: { email: email } });
        if (existingUser) {
            return res.status(409).json({ message: "Email already exists" });
        }

        const existingPhone = await models.user.findOne({ where: { phone: phone } });
        if (existingPhone) {
